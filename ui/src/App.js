...
const THEMES = {
  light: {
    pageBg: "#e8eaed", cardBg: "#f7f8fa", border: "#d5d8dd", borderInput: "#c3c8cf",
    neutralBg: "#dfe2e6", borderSubtle: "#b8bcc3", surfaceMuted: "#e3e6ea",
    textPrimary: "#1a1d23", textHeading: "#12141a", textMuted: "#6b7280", textDim: "#7d8590",
    neutralText: "#5b6572",
    accent: "#8a6d1f", accentSecond: "#96751f", accentCaption: "#8a7f5f",
    accentAlphaHover: "#8a6d1f44", accentAlphaSelect: "#8a6d1f30",
    hintBg: "#fdf6e3", selectedBg: "#f6ecd0", rowSelectBg: "#faf3df",
    hintTextGreen: "#3f7a52", successText: "#15803d", successBg: "#e6f5ea",
    dangerText: "#b23a3a", dangerHover: "#d13c3c", dangerBg: "#fdecec",
    dangerBorder: "#f3c8c8", dangerBadgeBg: "#fbe2e2", dangerMuted: "#8a4040",
    dangerBgOuter: "#fdf0f0",
    infoText: "#2f6a9a", infoBg: "#e8f1f8", infoBorder: "#bcdcee",
    purpleText: "#8a4fb0",
  },
  dark: {
    pageBg: "#080a0c", cardBg: "#0c0e12", border: "#181c24", borderInput: "#1e2530",
    neutralBg: "#0d0f12", borderSubtle: "#2a2e35", surfaceMuted: "#111418",
    textPrimary: "#d8d4cc", textHeading: "#e8e4da", textMuted: "#5a6575", textDim: "#4a5060",
    neutralText: "#555",
    accent: "#c8b560", accentSecond: "#c8a050", accentCaption: "#5a5040",
    accentAlphaHover: "#c8b56044", accentAlphaSelect: "#c8b56030",
    hintBg: "#3a3010", selectedBg: "#1a1608", rowSelectBg: "#0d1008",
    hintTextGreen: "#4a7a5a", successText: "#6bc47a", successBg: "#1e3a2a",
    dangerText: "#c46b6b", dangerHover: "#e05050", dangerBg: "#160b0b",
    dangerBorder: "#3a1414", dangerBadgeBg: "#3a1e1e", dangerMuted: "#3a2020",
    dangerBgOuter: "#120a0a",
    infoText: "#6b9bc4", infoBg: "#0a1420", infoBorder: "#1a3a4a",
    purpleText: "#a06bc4",
  },
};
...
function useTheme() {
  const [theme, setThemeState] = useState(() => {
    try { return localStorage.getItem("technion_theme") || "light"; } catch { return "light"; }
  });
  const setTheme = useCallback((next) => {
    setThemeState(next);
    try { localStorage.setItem("technion_theme", next); } catch {}
  }, []);
...
const ThemeToggle = ({ theme, onToggle }) => (
  <div
    ...
    style={{
      ...
      background: theme === "dark" ? "#2a2e35" : "#c3c8cf",
      position: "relative", transition: "background .2s",
    }}
  >
    <div style={{
      width: 14, height: 14, borderRadius: "50%", position: "absolute", top: 3,
      left: theme === "dark" ? 21 : 3, transition: "left .2s",
      background: theme === "dark" ? "#c8b560" : "#8a6d1f",
    }} />
  </div>
);
...
// ─── Storage (persisted to localStorage so courses survive app restarts) ──────
function useTakenCourses() {
  const [taken, setTakenState] = useState(() => {
    try {
      const saved = localStorage.getItem("technion_taken");
  ...
  const set = useCallback((updater) => {
    setTakenState((prev) => {
      ...
      try { localStorage.setItem("technion_taken", JSON.stringify(next)); } catch {}
      return next;
    });
  }, []);
  return [taken, set];
...
const GradePill = ({ g, theme }) => {
  ...
  let bg = T.successBg, color = T.successText;
  if (!isNaN(num) && num < 55) { bg = T.dangerBadgeBg; color = T.dangerText; }
  if (g === "Exemption –pts" || g === "Exemption") { bg = T.neutralBg; color = T.neutralText; }
  if (g === "In Progress") { bg = T.infoBg; color = T.infoText; }
  return (
...
const SourceDot = ({ s, theme }) => {
  const T = THEMES[theme];
  const label = s === "manual" ? "manual" : s === "current" ? "in progress" : "transcript";
  const color = s === "manual" ? T.accentSecond : s === "current" ? T.infoText : T.hintTextGreen;
...
export default function App() {
  ...
  // Check GitHub once on startup for a newer version than what's installed
  useEffect(() => {
    fetch("http://localhost:5000/api/update/check")
      .then(r => r.json())
      .then(data => { if (!data.error) setUpdateInfo(data); })
      .catch(() => {});
  }, []);
  ...
  const applyUpdate = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/update/apply", { method: "POST" });
  ...
  const [mustStatusKey, setMustStatusKey] = useState(null);
  ...
  const css = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    ::selection { background: ${T.accentAlphaSelect}; }
    ::-webkit-scrollbar { width: 6px; }
    ::-webkit-scrollbar-track { background: ${T.neutralBg}; }
    ...
    .row-hover:hover { background: ${T.surfaceMuted} !important; }
    .card-hover { transition: border-color .15s, transform .12s; cursor: pointer; }
    .card-hover:hover { border-color: ${T.accent} !important; transform: translateY(-1px); }
    ...
    .remove-btn:hover { color: ${T.dangerHover} !important; }
  ...
  const V = {
    wrap: { minHeight: "100vh", background: T.pageBg, color: T.textPrimary, fontFamily: "'IBM Plex Mono', monospace" },
    header: { borderBottom: `1px solid ${T.border}`, padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", background: T.pageBg, position: "sticky", top: 0, zIndex: 10 },
    logo: { fontSize: 12, letterSpacing: "0.18em", textTransform: "uppercase", color: T.textMuted },
    logoAccent: { color: T.accent },
    pill: { background: T.surfaceMuted, border: `1px solid ${T.borderInput}`, borderRadius: 4, padding: "3px 12px", fontSize: 11, color: T.textDim, letterSpacing: "0.06em" },
    main: { maxWidth: 820, margin: "0 auto", padding: "48px 24px 80px" },
    h1: { fontSize: 26, fontWeight: 400, letterSpacing: "-0.02em", color: T.textHeading, marginBottom: 6, fontFamily: "'IBM Plex Sans', sans-serif" },
    sub: { fontSize: 12, color: T.textMuted, marginBottom: 40, letterSpacing: "0.04em" },
    grid2: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 36 },
    card: { background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "22px 24px" },
    cardLabel: { fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: T.accent, marginBottom: 8 },
    cardDesc: { fontSize: 12, color: T.textMuted, lineHeight: 1.7 },
    section: { background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "24px", marginBottom: 20 },
    secTitle: { fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: T.textMuted, marginBottom: 16 },
    table: { width: "100%", borderCollapse: "collapse" },
    th: { fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: T.textDim, padding: "6px 10px", textAlign: "left", borderBottom: `1px solid ${T.border}` },
    td: { padding: "9px 10px", fontSize: 12, borderBottom: `1px solid ${T.surfaceMuted}`, verticalAlign: "middle" },
    dropzone: { border: `1.5px dashed ${T.borderInput}`, borderRadius: 8, padding: "52px 24px", textAlign: "center", cursor: "pointer", transition: "border-color .15s, background .15s" },
    dropActive: { borderColor: T.accent, background: T.hintBg },
    btnPrimary: { background: T.accent, color: T.pageBg, border: "none", borderRadius: 4, padding: "10px 22px", fontSize: 12, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600, letterSpacing: "0.06em", cursor: "pointer" },
    btnGhost: { background: "transparent", color: T.textMuted, border: `1px solid ${T.borderInput}`, borderRadius: 4, padding: "10px 22px", fontSize: 12, fontFamily: "'IBM Plex Mono', monospace", cursor: "pointer", transition: "all .15s" },
    input: { background: T.pageBg, border: `1px solid ${T.borderInput}`, borderRadius: 4, padding: "9px 12px", fontSize: 12, fontFamily: "'IBM Plex Mono', monospace", color: T.textPrimary, width: "100%", transition: "border-color .15s" },
    back: { fontSize: 11, color: T.textMuted, cursor: "pointer", letterSpacing: "0.08em", marginBottom: 32, display: "inline-flex", alignItems: "center", gap: 6, transition: "color .12s" },
    err: { background: T.dangerBg, border: `1px solid ${T.dangerBorder}`, borderRadius: 4, padding: "10px 14px", fontSize: 12, color: T.dangerText, marginBottom: 16 },
    toast: { position: "fixed", bottom: 28, left: "50%", transform: "translateX(-50%)", background: T.accent, color: T.pageBg, padding: "9px 22px", borderRadius: 4, fontSize: 12, fontWeight: 600, letterSpacing: "0.06em", zIndex: 999 },
  };
                  ...
                  <tbody>
                    {failedC.map((c) => (
                      <tr key={c.cid} style={V.td}>
                        <td style={{ ...V.td, color: T.textMuted }}>{c.cid}</td>
                        <td style={{ ...V.td, color: T.textDim }}>{c.name}</td>
                        <td style={V.td}><GradePill g={c.gradeStr} theme={theme} /></td>
                      </tr>
                    ))}
                  </tbody>
              ...
              >
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <div style={{ fontSize: 13, color: T.textDim }}>Drop PDF here or click to browse</div>
                    <div style={{ fontSize: 11, color: T.textDim, marginTop: 8, letterSpacing: "0.06em" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 16 }}>
                  {takenList.filter(([, info]) => info.source === "manual").map(([cid, info]) => (
                    <span key={cid} style={{ display: "flex", alignItems: "center", gap: 6,
                      ...
                      background: T.infoBg, border: `1px solid ${T.infoBorder}`, color: T.infoText }}>
                      <span className="he">{info.name || cid}</span>
                      <span style={{ cursor: "pointer", color: T.infoText }} onClick={() => removeCourse(cid)}>✕</span>
                    </span>
                  ))}
                </div>
              )}
  ...
  if (view === "plan") {
    ...
    // required courses' points always count toward the range — you can't
    // plan to take fewer points than you've already locked in as required
    const effectiveMin = Math.max(mustPts, minPts || 0);
    const effectiveMax = Math.max(mustPts, maxPts || 0);
    ...
    if (planStep === "semester") {
      ...
      return (
            ...
            <div className="fade-up">
              ...
              <div style={V.section}>
                ...
                {semStatus === "loading_list" ? (
                  <div style={{ fontSize: 12, color: T.textMuted }}>Loading semesters...</div>
                  ...
                  <>
                    <select value={semInput} onChange={e => setSemInput(e.target.value)}
                      ...
                      {semesterOptions.map(s => (
                        <option key={s.code} value={s.code} style={{ background: T.cardBg, color: T.accent }}>{s.label}</option>
                      ))}
                    </select>
                  ...
                  <div style={{ padding: "12px 16px", background: T.hintBg, border: `1px solid ${T.hintBg}`, borderRadius: 6 }}>
                    <div style={{ fontSize: 13, color: T.accent }}>⏳ No semester data found — run update_latest.py first</div>
                  </div>
                )}
              </div>
            </div>
        ...
        // ── Step: must-take ─────────────────────────────────────────────────────
    if (planStep === "must") {
      const mustKey = [...mustIds].sort().join(",");
      if (mustStatusKey !== mustKey && !mustStatusLoading) {
        ...
        fetch("http://localhost:5000/api/status", {
          ...
          .then(data => { setMustStatus(data); setMustStatusKey(mustKey); })
          .catch(() => { setMustStatus({}); setMustStatusKey(mustKey); })
      ...
      <div style={V.wrap}>
        ...
        <div style={V.main}>
          <div className="fade-up">
            ...
            <div style={V.sub}>Step 2 of 3 — add courses you must take this semester (optional)</div>
              <div style={{ fontSize: 11, color: T.hintTextGreen, marginTop: 6, marginBottom: 4 }}>💡 Have an exemption for a course like אנגלית טכנית? Add it to the <span style={{color:T.accent}}>exclude list</span> below so it won't be recommended</div>
            {err && <div style={V.err}>{err}</div>}
            ...
            {mustStatus && (
              <div style={V.section}>
                ...
                {reqRows.map(k => {
                  ...
                  return (
                    <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.textMuted, marginBottom: 8 }}>
                      <span className="he" style={{ cursor: "pointer", borderBottom: `1px dotted ${T.borderSubtle}` }}
                        onClick={() => setBrowseCategory(cat)}>
                        {REQ_LABELS[k]}
                      </span>
                      <span style={{ color: done ? T.successText : T.accent, fontWeight: 600 }}>
                        {done ? "✓ done" : `${missing[k]} ${k.endsWith("_n") ? (missing[k] === 1 ? "course" : "courses") : "pts"} needed`}
                      </span>
                    </div>
                  );
                })}
                <div style={{ fontSize: 10, color: T.textDim, marginTop: 4 }}>Click any category to browse its courses</div>
                <div style={{ fontSize: 10, color: T.textMuted, marginTop: 14, lineHeight: 1.6, borderTop: `1px solid ${T.border}`, paddingTop: 10 }}>
            ...
            <div style={V.section}>
              ...
              {mustIds.length > 0 ? (
                <>
                  <table style={V.table}>
                    ...
                    <tbody>
                      {mustIds.map(cid => {
                        ...
                        return (
                          <tr key={cid} className="row-hover">
                            <td style={{ ...V.td, color: T.accent }}>{cid}</td>
                            <td style={{ ...V.td, color: T.textMuted }} className="he">{c.name || "—"}</td>
                            <td style={{ ...V.td, color: T.textDim }}>{c.credits ?? "—"}</td>
                            <td style={{ ...V.td, textAlign: "right" }}>
                  ...
                  <div style={{ marginTop: 14, fontSize: 12, color: T.accent }}>
                    {mustPts} pts locked in
                  </div>
                </>
              ) : (
                <div style={{ fontSize: 12, color: T.textDim, padding: "16px 0" }}>
              ...
              <div style={{ marginTop: 28 }}>
                ...
                <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
                  ...
                  <button className="btn-ghost" style={{ ...V.btnGhost, borderColor:T.dangerMuted, color:T.dangerMuted }}
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:8 }}>
                    {blockIds.map(cid => {
                      ...
                      return (
                        <span key={cid} style={{ display:"flex", alignItems:"center", gap:6,
                          fontSize:11, padding:"4px 10px", borderRadius:3,
                          background:T.dangerBgOuter, border:`1px solid ${T.dangerBg}`, color:T.dangerMuted }}>
                          <span className="he">{c.name || cid}</span>
                          <span style={{ cursor:"pointer", color:T.dangerMuted }} onClick={() => removeBlock(cid)}>✕</span>
                        </span>
                      );
                    })}
                  </div>
        ...
        {browseCategory && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 100,
            ...
            <div style={{ background: T.cardBg, border: `1px solid ${T.borderInput}`, borderRadius: 8,
              padding: 24, maxWidth: 640, width: "100%", maxHeight: "80vh", display: "flex", flexDirection: "column" }}
              onClick={e => e.stopPropagation()}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                <div style={{ fontSize: 15, color: T.textHeading }} className="he">{browseCategory}</div>
                <span style={{ cursor: "pointer", color: T.textMuted, fontSize: 12 }} onClick={() => setBrowseCategory(null)}>✕ close</span>
              </div>
              <div style={{ fontSize: 11, color: T.textMuted, marginBottom: 14 }}>Courses you haven't taken yet</div>

              <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                <button
                  onClick={() => setBrowseNextOnly(true)}
                  style={{ ...V.btnGhost, padding: "6px 14px", fontSize: 11,
                    borderColor: browseNextOnly ? T.accent : T.borderInput, color: browseNextOnly ? T.accent : T.textMuted }}>
              ...
              <div style={{ overflow: "auto" }}>
                {browseNextOnly && semCourseLoading ? (
                  <div style={{ fontSize: 12, color: T.textMuted, padding: "16px 0" }}>Loading...</div>
                ) : browseRows.length === 0 ? (
                  <div style={{ fontSize: 12, color: T.textDim, padding: "16px 0" }}>
                      ...
                      {browseRows.map(([id, c]) => {
                        ...
                        return (
                          <tr key={id} className="row-hover">
                            <td style={{ ...V.td, color: T.accent, letterSpacing: "0.05em" }}>{id}</td>
                            <td style={{ ...V.td, color: T.textDim }} className="he">{c.name}</td>
                            <td style={{ ...V.td, color: T.textDim }}>{c.credits ?? "—"}</td>
                            <td style={{ ...V.td, color: c.avgGrade ? T.successText : T.textMuted }}>{c.avgGrade ? c.avgGrade.toFixed(1) : "—"}</td>
                            <td style={{ ...V.td, color: T.textMuted }}>{c.avgRank ? c.avgRank.toFixed(2) : "—"}</td>
                            <td style={{ ...V.td, fontSize: 11, color: c.hasTest === "1" ? T.dangerText : c.hasTest === "0" ? T.successText : T.textDim }}>
                              {c.hasTest === "1" ? "exam" : c.hasTest === "0" ? "no exam" : "—"}
                            </td>
                            <td style={V.td}>
                              <span
                                style={{ cursor: "pointer", fontSize: 10, letterSpacing: "0.04em", color: isMust ? T.accent : T.textDim }}
                            ...
                            <td style={V.td}>
                              <span
                                style={{ cursor: "pointer", fontSize: 10, letterSpacing: "0.04em", color: isBlocked ? T.dangerText : T.textDim }}
        ...
        <div style={V.wrap}>
          ...
          <div style={V.main}>
            <div className="fade-up">
              ...
              <div style={V.section}>
                {mustPts > 0 && (
                  <div style={{ marginBottom: 20, padding: "10px 14px", background: T.hintBg, border: `1px solid ${T.hintBg}`, borderRadius: 6, fontSize: 12, color: T.accent }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-end", gap: 20, marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 10, color: T.textMuted, marginBottom: 6, letterSpacing: "0.08em" }}>MIN PTS</div>
                    <input type="number" min={mustPts} step={0.5}
                      value={effectiveMin}
                      onChange={e => setMinPts(e.target.value === "" ? "" : parseFloat(e.target.value))}
                      style={{ ...V.input, width: 110, fontSize: 20, textAlign: "center", color: T.accent, fontWeight: 500 }} />
                    {mustPts > 0 && <div style={{ fontSize: 10, color: T.textMuted, marginTop: 6 }}>+{remaining(effectiveMin)} free</div>}
                  </div>
                  <div style={{ fontSize: 18, color: T.textDim, paddingBottom: 10 }}>–</div>
                  <div>
                    <div style={{ fontSize: 10, color: T.textMuted, marginBottom: 6, letterSpacing: "0.08em" }}>MAX PTS</div>
                    <input type="number" min={mustPts} step={0.5}
                      value={effectiveMax}
                      onChange={e => setMaxPts(e.target.value === "" ? "" : parseFloat(e.target.value))}
                      style={{ ...V.input, width: 110, fontSize: 20, textAlign: "center", color: T.accent, fontWeight: 500 }} />
                    {mustPts > 0 && <div style={{ fontSize: 10, color: T.textMuted, marginTop: 6 }}>+{remaining(effectiveMax)} free</div>}
                  </div>
                </div>

                <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 24 }}>
                  Planning <span style={{ color: T.accent }}>{effectiveMin} – {effectiveMax} pts</span> for {semLabel(semInput)}
                  {mustPts > 0 && <span style={{ color: T.accentCaption }}> ({mustPts} required + {remaining(effectiveMin)}–{remaining(effectiveMax)} elective)</span>}
                </div>
                  ...
                  {EXAM_PREF_CATEGORIES.map(({ cat, label }) => {
                    ...
                    return (
                      <div key={cat} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                        <span style={{ fontSize: 12, color: T.textMuted }} className="he">{label}</span>
                          ...
                          {[["any", "Any"], ["with", "With exam"], ["without", "No exam"]].map(([v, text]) => (
                            <button key={v}
                              ...
                              style={{
                                ...
                                border: `1px solid ${val === v ? T.accent : T.borderInput}`,
                                background: val === v ? T.selectedBg : "transparent",
                                color: val === v ? T.accent : T.textMuted,
                ...
                {recLoading ? (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <span className="spin" style={{ fontSize: 20, color: T.accent }}>◐</span>
                      <span style={{ fontSize: 13, color: T.accent, letterSpacing: "0.06em" }}>Finding best schedule...</span>
                    </div>
                    ...
                    <div style={{ fontSize: 11, color: T.textMuted }}>Checking prerequisites · optimizing combinations</div>
                  </div>
                ) : (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:28 }}>
                {recOptions.map((opt, i) => (
                  <div key={i} onClick={() => selectOption(opt)}
                    style={{ background:T.cardBg, border:`1px solid ${T.borderInput}`, borderRadius:8,
                             padding:"16px 20px", cursor:"pointer", display:"flex", alignItems:"center", gap:0,
                             transition:"border-color .15s" }}
                    onMouseEnter={e => e.currentTarget.style.borderColor=T.accentAlphaHover}
                    onMouseLeave={e => e.currentTarget.style.borderColor=T.borderInput}>

                    {/* Stat pills */}
                    <div style={{ flex:"0 0 72px", textAlign:"center", borderRight:`1px solid ${T.borderInput}`, paddingRight:16, marginRight:16 }}>
                      <div style={{ fontSize:10, color:T.textMuted, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:4 }}>pts</div>
                      <div style={{ fontSize:24, color:T.accent, fontWeight:600, lineHeight:1 }}>{opt.total_credits}</div>
                    </div>
                    <div style={{ flex:"0 0 60px", textAlign:"center", borderRight:`1px solid ${T.borderInput}`, paddingRight:16, marginRight:16 }}>
                      <div style={{ fontSize:10, color:T.textMuted, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:4 }}>courses</div>
                      <div style={{ fontSize:24, color:T.accent, fontWeight:600, lineHeight:1 }}>{opt.n_courses}</div>
                    </div>
                    <div style={{ flex:"0 0 72px", textAlign:"center", borderRight:`1px solid ${T.borderInput}`, paddingRight:16, marginRight:16 }}>
                      <div style={{ fontSize:10, color:T.textMuted, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:4 }}>avg</div>
                      <div style={{ fontSize:24, fontWeight:600, lineHeight:1,
                                    color: opt.weighted_grade>=85?T.successText:opt.weighted_grade>=70?T.accent:T.dangerText }}>
                    ...
                    <div style={{ flex:1, display:"flex", flexWrap:"wrap", gap:5 }}>
                      {opt.courses.map(c => (
                        <span key={c.id} className="he" style={{ fontSize:11, padding:"3px 8px", borderRadius:3,
                          background:T.surfaceMuted, border:`1px solid ${catColor(c.category)}33`,
                          color: c.must ? T.accent : catColor(c.category), whiteSpace:"nowrap" }}>
                          {c.name || c.id}
                        </span>
                      ))}
                    </div>
                    <div style={{ color:T.textMuted, fontSize:14, paddingLeft:12 }}>&#x2192;</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={V.main}>
            <div className="fade-up">
              ...
              <div style={{ display: "flex", gap: 20, marginBottom: 28, flexWrap: "wrap" }}>
                <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "14px 24px", textAlign: "center" }}>
                  <div style={{ fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", color: T.textMuted, marginBottom: 6 }}>Total credits</div>
                  <div style={{ fontSize: 28, color: T.accent, fontWeight: 500 }}>{recommendation.total_credits}</div>
                </div>
                <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "14px 24px", textAlign: "center" }}>
                  <div style={{ fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", color: T.textMuted, marginBottom: 6 }}>Est. weighted avg</div>
                  <div style={{ fontSize: 28, fontWeight: 500, color: recommendation.weighted_grade >= 85 ? T.successText : recommendation.weighted_grade >= 70 ? T.accent : T.dangerText }}>
                    {recommendation.weighted_grade > 0 ? recommendation.weighted_grade.toFixed(1) : "—"}
                  </div>
                </div>
                <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "14px 24px", textAlign: "center" }}>
                  <div style={{ fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", color: T.textMuted, marginBottom: 6 }}>Courses</div>
                  <div style={{ fontSize: 28, color: T.accent, fontWeight: 500 }}>{recommendation.courses.length}</div>
                </div>
              </div>
                  ...
                  <tbody>
                    {recommendation.courses.map(c => (
                      <tr key={c.id} className="row-hover">
                        <td style={{ ...V.td, color: T.accent, letterSpacing: "0.05em" }}>{c.id}</td>
                        <td style={{ ...V.td, color: T.textDim }} className="he">{c.name}</td>
                        <td style={{ ...V.td }}>
                          <span style={{ color: catColor(c.category), fontSize: 11 }} className="he">{c.category}</span>
                        </td>
                        <td style={{ ...V.td, color: T.textDim }}>{c.credits}</td>
                        <td style={{ ...V.td, color: c.grade ? T.successText : T.textMuted }}>
                          {c.grade ? c.grade.toFixed(1) : "—"}
                        </td>
                        <td style={{ ...V.td, fontSize: 11, color: c.has_test === "1" ? T.dangerText : c.has_test === "0" ? T.successText : T.textDim }}>
                          {c.has_test === "1" ? `exam${c.exam_date_a ? " · " + c.exam_date_a : ""}` : c.has_test === "0" ? "no exam" : "—"}
                        </td>
                        <td style={{ ...V.td }}>
                          {c.must && <span style={{ fontSize: 10, color: T.accent, letterSpacing: "0.06em" }}>REQUIRED</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                    ...
                    <div style={V.section}>
                      ...
                      {rows.map(k => (
                        <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                          <span style={{ fontSize: 12, color: T.textMuted }} className="he">{REQ_LABELS[k]}</span>
                          <span style={{ fontSize: 13, color: before[k] > 0 ? T.dangerText : T.successText, fontWeight: 600 }}>
                            {before[k] > 0 ? `-${before[k]}` : "✓"}
                          </span>
                        </div>
                      ))}
                        ...
                        return (
                          <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              {diff > 0 && <span style={{ fontSize: 10, color: T.successText }}>+{diff}</span>}
                              <span style={{ fontSize: 13, color: after[k] > 0 ? T.dangerText : T.successText, fontWeight: 600 }}>
                                {after[k] > 0 ? `-${after[k]}` : "✓"}
                              </span>
                            </div>
                          </div>
      ...
      <div style={V.main}>
        <div className="fade-up">
          ...
          {updateInfo?.update_available && !updateDone && (
            <div style={{ ...V.section, borderColor: T.hintBg, background: T.hintBg, marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: T.accent, marginBottom: 6 }}>
                ⬆ An update is available{updateInfo.latest_message ? ` — "${updateInfo.latest_message}"` : ""}
              </div>
              {updateError && <div style={{ fontSize: 11, color: T.dangerText, marginBottom: 8 }}>{updateError}</div>}
              <button className="btn-primary" style={V.btnPrimary} onClick={applyUpdate} disabled={updateApplying}>
                {updateApplying ? "Updating..." : "Update now →"}
              </button>
            </div>
          )}
          {updateDone && (
            <div style={{ ...V.section, borderColor: T.successBg, background: T.successBg, marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: T.successText }}>
                ✓ Updated! Close this window and re-run "Start Technion Tracker" to load the new version.
              </div>
            </div>
          )}

          {dbLoading && (
            <div style={{ ...V.section, borderColor: T.borderInput, marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: T.textMuted }}>Loading course database...</div>
            </div>
          )}
          {!dbLoading && Object.keys(coursesDb).length === 0 && (
            <div style={{ ...V.section, borderColor: T.hintBg, background: T.hintBg, marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: T.accent }}>
                ⚠ Could not load course database — make sure <span style={{ color: T.accent }}>courses_labeled.csv</span> is in <span style={{ color: T.accent }}>ui/public/</span>
              </div>
            </div>
          )}
          {!dbLoading && Object.keys(coursesDb).length > 0 && (
            <div style={{ fontSize: 11, color: T.hintTextGreen, marginBottom: 20, letterSpacing: "0.06em" }}>
